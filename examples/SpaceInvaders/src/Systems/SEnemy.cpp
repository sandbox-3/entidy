...
void SEnemy::Init(Engine engine)
{
	...
	engine->Entidy()->Emplace<int>(e, "EnemySpawn", 1);
}

void SEnemy::Update(Engine engine)
{
	auto spawn_view = engine->Entidy()->Select({"EnemySpawn"}).Having("EnemySpawn");
    ...
    if((*spawn)-- == 0)
    {
        *spawn = 512 + Helper::RandInt(0, 256);
        ...
        for(int i = 0; i < count; i++)
        {
            ...
            Entity e = engine->Entidy()->Create();
            engine->Entidy()->Emplace<Vec2f>(e, "Position", sep_dist * i, 3);
            engine->Entidy()->Emplace<Sprite>(e, "Sprite", SpriteFactory::Enemy(enemy_type));
            engine->Entidy()->Emplace<Vec2f>(e, "Velocity", dir, 0.01);
            engine->Entidy()->Emplace<BoundaryAction>(e, "BoundaryAction", BoundaryAction::WARP);
            engine->Entidy()->Emplace<u_int8_t>(e, "Enemy");
            engine->Entidy()->Emplace<int>(e, "Health", (enemy_type + 1) * 20);
        }
    }

	auto sprite_view = engine->Entidy()->Select({"Sprite", "Position", "Health"}).Having("Sprite & Position & Health & Enemy");
	sprite_view.Each([&](Entity enemy, Sprite* sprite, Vec2f* enemy_pos, uint8_t * health) {
		auto bullet_view = engine->Entidy()->Select({"Position"}).Having("Bullet & Position");
		bullet_view.Each([&](Entity bullet, Vec2f* bullet_pos) {
            ...
            {
                Entity ripple_entity = engine->Entidy()->Create();
                BGFXRipple ripple;
                ripple.center = Vec2f(bullet_pos->x, bullet_pos->y);
                ripple.radius = 0;

                engine->Entidy()->Erase(bullet);
                (*health)--;
                if(*health == 0)
                {
                    engine->Entidy()->Erase(enemy);
                    ripple.intensity = 1;
                }
                else
                {
                ...
                engine->Entidy()->Emplace(ripple_entity, "BGFXRipple", ripple);
            }
        });
    });
}
